...
class BARDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if label_str not in self.class_to_idx:
            raise ValueError(
                f"Unrecognized class '{label_str}' parsed from filename '{img_path.name}'. Expected one of : {self.CLASSES}"
            )
...
def create_validation_splits(
    train_dir: Path, test_dir: Path, num_samples_per_class: int, seed: int
    ...
    for activity, images in train_activity_images.items():
        if len(images) < num_samples_per_class:
            print(
                f"Warning: {activity} has only {len(images)} images in the training set, requested {num_samples_per_class}"
            )
            sampled = images
        else:
    ...
    for activity, images in test_activity_images.items():
        if len(images) < num_samples_per_class:
            print(
                f"Warning: {activity} has only {len(images)} images in the test set, requested {num_samples_per_class}"
            )
            sampled = images
        else:
            sampled = random.sample(images, num_samples_per_class)
...
def get_bar_dataloaders(
    ...
    out_of_domain_val_dataset = BARDataset(
        out_of_domain_val_paths, transform=data_transform
    )
