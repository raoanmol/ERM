...
def parse_celeba_metadata(
    data_root: Path, target_attr: str
    ...
    if target_attr not in header:
        raise ValueError(
            f"Target attribute '{target_attr}' not found. Available attributes: {header}"
        )
...
def get_celeba_dataloaders(
    ...
    in_domain_val_dataset = CelebADataset(
        in_domain_val_paths, in_domain_val_labels, transform=data_transform
    )
    out_of_domain_val_dataset = CelebADataset(
